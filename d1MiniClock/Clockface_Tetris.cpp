/*
   Tetris Clockface
   A clockface that shows an animation of Tetris.
*/
...
const uint8_t PROGMEM gfx[PIECE_NUM * PIECE_MAX_HEIGHT * 4] = {
  // Square
  0b00000011
  , 0b00000011
  , 0b00110000
...
bool ClockfaceTetris::checkCollision(int8_t xd, int8_t yd, uint8_t r) const {
  ...
  for (uint8_t iy = 0; iy < PIECE_MAX_HEIGHT; ++iy) {
    uint8_t row = pgm_read_byte(gfx + (piece * 4 + r) * PIECE_MAX_HEIGHT + iy);
    for (uint8_t ix = 0; ix < PIECE_MAX_WIDTH; ++ix) {
      if (row & _BV(ix)) {
        // Bounds
...
// Render the current piece onto the board bitmap
void ClockfaceTetris::tileToBoard() {
  for (uint8_t iy = 0; iy < PIECE_MAX_HEIGHT; ++iy) {
    uint8_t row = pgm_read_byte(gfx + (piece * 4 + rotation) * PIECE_MAX_HEIGHT + iy);
    for (uint8_t ix = 0; ix < PIECE_MAX_WIDTH; ++ix) {
      if (row & _BV(ix)) {
        board[y + iy] |= _BV(x + ix);
      }
...
void ClockfaceTetris::clearLines() {
  // Generate a bitmask with all 1s where we expect a piece.
...
// Find out the height of a piece.
// Used in the AI only.
uint8_t ClockfaceTetris::getPieceHeight(uint8_t piece, uint8_t r) const {
  if (piece == 0) {
    // Square
    return 2;
  } else if (piece == 1) {
...
uint8_t ClockfaceTetris::blockingMetric(int8_t xd, int8_t yd, uint8_t r) const {
  ...
  for (int8_t iy = 0; iy < PIECE_MAX_HEIGHT; ++iy) {
    uint8_t row = pgm_read_byte(gfx + (piece * 4 + r) * PIECE_MAX_HEIGHT + iy);
    for (int8_t ix = 0; ix < PIECE_MAX_WIDTH; ++ix) {
      if (row & _BV(ix + 4)) {
        // Bounds
...
void ClockfaceTetris::decideMove() {
  ...
  for (uint8_t r = 0; r < 4; ++r) {
    uint8_t pieceHeight = getPieceHeight(piece, r);
    for (int8_t i = 0; i < BOARD_WIDTH; ++i) {
      ...
      for (int8_t iy = 1; iy < BOARD_HEIGHT; ++iy) {
        ...
        if (checkCollision(i - x, iy, r)) {
          ...
          int8_t score = pieceHeight + iy - blockingMetric(i - x, iy - 1, r);
          if (score > bestScore) {
            bestScore = score;
            bestX = i;
            bestR = r;
