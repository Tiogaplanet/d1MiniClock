/*
   Pong Clockface
   Plays a game of Pong and shows the time as if it were the score.

   The code and algorithms originated from https://github.com/rparrett/pongclock
...
void ClockfacePong::update(uint8_t hour, uint8_t minute) {
  // If paused, don't do any movements
  if (pause > 0) {
    --pause;
    return;
  }
...
void ClockfacePong::drawScore(Adafruit_GFX* display) const {
  ...
  if (lscore > 9)
    lscore_x = WIDTH / 2 - font_w - font_w - font_sp - 10;
  else {
    lscore_x = WIDTH / 2 - font_w - font_w - font_sp - 8;
    display->setCursor(lscore_x, 4 / 2);
    display->print('0');
    lscore_x = WIDTH / 2 - font_w - 10;
  }
  ...
  if (rscore < 10){
    display->println('0');
    display->setCursor(rscore_x + 12, 4 / 2);
  }
